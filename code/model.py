...
class ImageCaptionModel(tf.keras.Model):
    ...
    def train(self, train_captions, train_image_features, padding_index, batch_size=30):
        ...
        #index = tf.range(len(train_captions, dtype=tf.int32))
        shuffle = tf.random.shuffle(tf.range(len(train_captions)))
        train_captions = tf.gather(train_captions, shuffle)
        train_image_feature = tf.gather(train_image_features, shuffle)
